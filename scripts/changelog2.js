...
class ApiDataCache {
  constructor(host, config) {
    this.host = host;
    this.dir = config.cacheDir && path.join(config.rootPath, config.cacheDir, host);

    if (this.dir) {
      try {
        mkdirp.sync(this.dir);
      } catch (error) {
        throw new Error(`Can't use cacheDir "${config.cacheDir}" (${error.message})`);
      }
    }
  }
...
class GithubAPI {
  ...
  _fetch(type, key) {
    const url = {
      issue: `https://api.github.com/repos/${this.repo}/issues/${key}`,
...
class Changelog {
  ...
  getConfig() {
    ...
    if (!config) {
      throw new Error("Missing changelog config in `lerna.json`.");
    }
  ...
  getListOfUniquePackages(sha) {
    return Object.keys(
      // turn into an array
      execSync("git show -m --name-only --pretty=\"format:\" --first-parent " + sha).split("\n").reduce((acc, files) => {
